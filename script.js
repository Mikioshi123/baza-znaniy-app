document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners(data) {
        ...
        function performSearch() {
            ...
            // Этап 1: Ищем точное вхождение всей фразы
            ...
            // Этап 2: Если точных совпадений нет, ищем записи, содержащие ВСЕ слова
            if (searchResult.length === 0) {
                const searchWords = searchTerm.split(' ').filter(word => word.length >= fuseOptions.minMatchCharLength);
                if (searchWords.length > 0) {
                    const andMatches = dataToSearch.filter(item =>
                        searchWords.every(word => item.question.toLowerCase().includes(word))
                    );
                    if (andMatches.length > 0) {
                        searchResult = andMatches;
                    }
                }
            }

            // Этап 3: Если и так ничего не найдено, используем "умный" поиск Fuse.js по ЛЮБОМУ слову
            if (searchResult.length === 0) {
                ...
                if (extendedSearchTerm) {
                    searchResult = fuseInstance.search(extendedSearchTerm);
                }
            }
