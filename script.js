document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderResults(results) {
        // ... (этот код из предыдущего сообщения не меняется)
    }
                ...
                tg.showConfirm(`Вы уверены, что хотите купить этот товар за ${itemPrice} MC?`, async (confirmed) => {
                    if (confirmed) {
                        try {
                            ...
                            gamificationData.currentUser.mc_balance = data.newBalance; // Обновляем баланс локально
                            renderMainLayout(currentUser.first_name, gamificationData.currentUser.mc_balance); // Перерисовываем шапку
                            renderMarketplaceTab(); // Перерисовываем магазин с обновленными кнопками
    ...
    // Полный копипаст всех функций для избежания ошибок
