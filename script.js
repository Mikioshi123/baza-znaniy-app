document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners(data) {
        ...
        function performSearch() {
            ...
            const rankedResults = dataToSearch.map(item => {
                const question = item.question.toLowerCase();
                let score = 0;

                if (question === searchTerm) { // Уровень 1: Точное совпадение
                    score = 100;
                } else if (question.includes(searchTerm)) { // Уровень 2: Фраза как часть
                    score = 50;
                } else if (searchWords.length > 0 && searchWords.every(word => question.includes(word))) { // Уровень 3: Все слова есть
                    score = 25;
                }
                return { ...item, score };
            }).filter(item => item.score > 0);

            // Уровень 4: "Умный" поиск
            if (searchWords.length > 0) {
                const fuse = new Fuse(dataToSearch, fuseOptions);
                const fuzzyMatches = fuse.search(searchTerm);

                fuzzyMatches.forEach(result => {
                    if (!rankedResults.find(r => r.id === result.item.id)) {
                        rankedResults.push({ ...result.item, score: 10 - (result.score * 10) });
                    }
                });
            }
