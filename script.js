document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (весь код до функции saveData без изменений)
    ...
    function saveData(objectionId, dataToSave) {
        ...
        saveTimeout = setTimeout(async () => {
            try {
                ...
                // --- НОВЫЙ БЛОК: ОБНОВЛЯЕМ ИНТЕРФЕЙС, ЕСЛИ ПРИШЕЛ НОВЫЙ РЕЙТИНГ ---
                if (data.newAverageRating !== null) {
                    // Обновляем данные локально, чтобы при поиске все было актуально
                    userPersonalData.ratings[objectionId] = data.newAverageRating;
                    // Находим нужный блок со звездами и перерисовываем его
                    const starsContainer = document.querySelector(`.rating-stars[data-objection-id="${objectionId}"]`);
                    if (starsContainer) {
                        starsContainer.querySelectorAll('.star').forEach(star => {
                            star.classList.toggle('filled', parseInt(star.dataset.value) <= data.newAverageRating);
                        });
                    }
                }
            } catch (error) {
        ...
        document.querySelectorAll('.rating-stars').forEach(starsContainer => {
            starsContainer.addEventListener('click', (e) => {
                if (e.target.classList.contains('star')) {
                    ...
                    userPersonalData.ratings[objectionId] = ratingValue;
                    starsContainer.querySelectorAll('.star').forEach(star => {
