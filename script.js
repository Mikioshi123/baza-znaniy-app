document.addEventListener('DOMContentLoaded', () => {
    ...
    async function saveData(objectionId, noteText, ratingValue) {
        try {
            ...
            main();
        } catch (error) { console.error("Failed to save data:", error); tg.showAlert('Ошибка сохранения.'); }
    }
    ...
    async function deleteData(objectionId) {
        try {
            ...
            main();
        } catch (error) { console.error("Failed to delete data:", error); tg.showAlert('Ошибка удаления.'); }
    }
    ...
    function setupEventListeners(data) {
        ...
        const fuseOptions = {
            keys: ['question'],
            includeScore: true,
            threshold: 0.4, // Немного повысим порог для лучшего поиска по опечаткам
        ...
        function performSearch() {
            ...
            // 1. Сначала фильтруем данные по выбранной категории
            let dataToSearch = (currentFilter === 'all')
                ? data
                : data.filter(item => item.category === currentFilter);

            // 2. Если поисковая строка пуста, показываем все отфильтрованные данные
            if (searchTerm === '') {
                renderResults(dataToSearch);
                return;
            }

            // 3. Используем Fuse.js для "умного" поиска по отфильтрованным данным
            const fuse = new Fuse(dataToSearch, fuseOptions);
            const fuseResults = fuse.search(searchTerm);

            // 4. Сортируем результаты: сначала те, где есть точное вхождение фразы
            fuseResults.sort((a, b) => {
                const aIncludes = a.item.question.toLowerCase().includes(searchTerm);
                const bIncludes = b.item.question.toLowerCase().includes(searchTerm);

                if (aIncludes && !bIncludes) return -1; // "a" важнее, идет вверх
                if (!aIncludes && bIncludes) return 1;  // "b" важнее, идет вверх
                return a.score - b.score; // Если оба (не)содержат, сортируем по релевантности Fuse
            });
