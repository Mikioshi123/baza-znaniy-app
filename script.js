    ...
    function setupFeedbackListener() {
        ...
        if (feedbackBtn) {
            feedbackBtn.addEventListener('click', (e) => {
                ...
                // Используем метод showPrompt, который специально предназначен для ввода текста
                tg.showPrompt(
                    `Опишите, какой отработки вам не хватает для запроса "${currentSearch}"?`,
                    async (text) => {
                        // Эта функция вызовется, когда пользователь нажмет "ОК"
