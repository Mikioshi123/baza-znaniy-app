document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupFeedbackListener() {
        ...
        if (feedbackBtn) {
            feedbackBtn.addEventListener('click', async (e) => { // Добавляем async сюда
                const currentSearch = e.target.dataset.searchTerm;

                // Используем стандартный браузерный prompt
                const userComment = prompt(`Опишите, какой отработки вам не хватает для запроса "${currentSearch}"?`);

                // Проверяем, что пользователь что-то ввел и не нажал "Отмена"
                if (userComment && userComment.trim().length > 0) {
                    try {
                        // Показываем простое уведомление о начале отправки
                        tg.showAlert('Отправляем ваш запрос...');

                        await supabaseClient.functions.invoke('save-feedback', {
                            body: { userId: currentUser.id, searchQuery: currentSearch, comment: userComment }
                        });

                        tg.showAlert('Спасибо! Ваш запрос успешно отправлен.');
                    } catch (error) {
