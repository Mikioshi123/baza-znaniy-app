document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners(data) {
        ...
        function performSearch() {
            ...
            const searchWords = searchTerm.split(' ').filter(word => word.length >= fuseOptions.minMatchCharLength);

            // Этап 1: Точное вхождение фразы
            const exactMatches = dataToSearch.filter(item =>
                item.question.toLowerCase().includes(searchTerm)
            );

            // Этап 2: Все слова из запроса
            const allWordsMatches = dataToSearch.filter(item =>
                searchWords.length > 0 && searchWords.every(word => item.question.toLowerCase().includes(word))
            );

            // Этап 3: "Умный" поиск по любому слову
            const fuseInstance = new Fuse(dataToSearch, fuseOptions);
            const extendedSearchTerm = searchWords.map(word => `'${word}`).join(' | ');
            const fuzzyMatchesRaw = extendedSearchTerm ? fuseInstance.search(extendedSearchTerm) : [];
            const fuzzyMatches = fuzzyMatchesRaw.map(result => result.item);

            // Собираем финальный результат с приоритетами
            const finalResults = [
                ...exactMatches,
                ...allWordsMatches,
                ...fuzzyMatches
