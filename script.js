document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderResults(results) {
        ...
        results.forEach(item => {
            // В этой версии item.item не будет, так как мы не используем Fuse.js
            const record = item;
    ...
    function setupEventListeners(data) {
        ...
        function performSearch() {
            ...
            // 2. Оставляем только самый простой поиск на точное вхождение
            const searchResult = dataToSearch.filter(item =>
